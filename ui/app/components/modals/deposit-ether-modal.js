...
function mapDispatchToProps (dispatch) {
  return {
    toCoinbase: (address) => {
      dispatch(actions.buyEth({ network: '1', address, amount: '' }))
    },
    toPlayWKC: (address) => {
      dispatch(actions.buyEth({ network: '2', address, amount: '' }))
    },
    hideModal: () => {
...
DepositEtherModal.prototype.render = function () {
  const { network, toCoinbase, address, toFaucet, toPlayWKC} = this.props
    ...
    h('.page-container__content', {}, [
      ...
      h('div.deposit-ether-modal__buy-rows', [
        ...
        this.renderRow({
          logo: h('div.deposit-ether-modal__logo', {
            style: {
              backgroundImage: 'url(\'./images/playwkc.png\')',
            },
          }),
          title: SHAPESHIFT_ROW_TITLE,
          text: SHAPESHIFT_ROW_TEXT,
          buttonLabel: this.context.t('shapeshiftBuy'),
          onButtonClick: () => toPlayWKC(address),
          hide: isTestNetwork ,
        })
      ]),
