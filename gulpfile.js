...
function createTasksForBuildJsExtension ({ buildJsFiles, taskPrefix, devMode, bundleTaskOpts = {} }) {
  // lkpage must be built before all other scripts:
  const rootDir = './app/scripts'
  const nonInpageFiles = buildJsFiles.filter(file => file !== 'lkpage')
  const buildPhase1 = ['lkpage']
...
function zipTask (target) {
  return () => {
    ...
    .pipe(zip(`linker-${target}-${manifest.version}.zip`))
...
function bundleTask (opts) {
  ...
  function performBundle () {
    ...
    if (opts.minifyBuild) {
      ...
      .pipe(uglify({
        mangle: {
          reserved: [ 'LinkTokenLkpageProvider','LkLinkTokenLkpageProvider' ],
        },
      }))
    }
