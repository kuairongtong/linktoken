...
module.exports = class MetamaskController extends EventEmitter {
  ...
  initializeProvider () {
    const providerOpts = {
      static: {
        eth_syncing: false,
        linktoken_clientVersion: `Linker/v${version}`,
      },
      version,
      // account mgmt
      getAccounts: async ({ origin }) => {
        ...
        if (origin !== 'Linker' && !exposeAccounts) { return [] }
...
//=============================================================================
  ...
  getBalance (address, ethQuery) {
    return new Promise((resolve, reject) => {
      ...
      if (cached && cached.balance) {
        ...
        ethQuery.getBalance(address,'pending',(error, balance) => {
          if (error) {
            reject(error)
            log.error(error)
          } else {
   ...
   * Each strategy represents a different way of serializing an LinkToken key pair.
   *
  ...
  signMessage (msgParams) {
    log.info('LinkerController - signMessage')
  ...
  signPersonalMessage (msgParams) {
    log.info('LinkerController - signPersonalMessage')
    const msgId = msgParams.metamaskId
    // sets the status op the message to 'approved'
    // and removes the metamaskId for signing
  ...
  async signTypedMessage (msgParams) {
    log.info('LinkerController - eth_signTypedData')
    const msgId = msgParams.metamaskId
    ...
    try {
      ...
      log.info('LinkerController - eth_signTypedData failed.', error)
...
//=============================================================================
  ...
  estimateGas (estimateGasParams) {
    return new Promise((resolve, reject) => {return resolve('0x186a0')})
  }
...
//=============================================================================
  ...
  setupUntrustedCommunication (connectionStream, originDomain) {
    ...
    if (this.blacklistController.checkForPhishing(originDomain)) {
      log.debug('Linker - sending phishing warning for', originDomain)
...
//=============================================================================
  ...
  setCurrentCurrency (currencyCode, cb) {
    ...
    try {
      ...
      this.currencyController.updateConversionRate()
      const data = {
        nativeCurrency: ticker || 'LinkToken',
   ...
   * @param {string} rpcTarget - A URL for a valid LinkToken RPC API.
