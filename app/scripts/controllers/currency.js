  ...
  async updateConversionRate () {
    ...
    try {
      ...
      if (currentCurrency === 'cny') {
        apiUrl = `http://price-linktoken.krtnt.com/api/ticker?symbol=wkc`
      } else {
       // ETC
      ...
      // set conversion rate
      if ( currentCurrency === 'cny') {
        this.setConversionRate(Number(parsedResponse.ticker.last))
        this.setConversionDate(Number(parsedResponse.time))
      } else {
        // ETC
      ...
      log.warn(`Linker - Failed to query currency conversion:`, nativeCurrency, currentCurrency, err)
      this.setConversionRate(0)
      this.setConversionDate('N/A')
      // throw error
