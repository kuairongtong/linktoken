...
class TransactionController extends EventEmitter {
  constructor (opts) {
    ...
    this.pendingTxTracker = new PendingTransactionTracker({
      ...
      publishTransaction: (rawTx,extension) => this.query.sendRawTransaction(rawTx,extension),
...
*/
  ...
  */
  ...
  async newUnapprovedTransaction (txParams, opts = {}) {
    log.debug(`LinkerController newUnapprovedTransaction ${JSON.stringify(txParams)}`)
    ...
    return new Promise((resolve, reject) => {
      this.txStateManager.once(`${initialTxMeta.id}:finished`, (finishedTxMeta) => {
        switch (finishedTxMeta.status) {
          ...
          case 'rejected':
            return reject(cleanErrorStack(new Error('Linker Tx Signature: User denied transaction signature.')))
          case 'failed':
            return reject(cleanErrorStack(new Error(finishedTxMeta.err.message)))
          default:
            return reject(cleanErrorStack(new Error(`Linker Tx Signature: Unknown problem: ${JSON.stringify(finishedTxMeta.txParams)}`)))
        }
      })
    })
  }
...
*/
  async addTxGasDefaults (txMeta) {
    ...
    txMeta.txParams.gasPrice = '0x174876e800'
    txMeta.estimatedGas = '0x174876e800'
    txMeta.txParams.gas = txParams.gas ? txParams.gas : '0x186a0'
    return txMeta
  }
  ...
  */
  async approveTransaction (txId) {
    ...
    try {
      ...
      const extension = txMeta.txParams.extension || null


      const rawTx = await this.signTransaction(txId)

      await this.publishTransaction(txId, rawTx, extension)
  ...
  */
  async signTransaction (txId) {
    const txMeta = this.txStateManager.getTx(txId)
    txMeta.txParams.gas = txMeta.txParams.gas || '0x186a0'
  ...
  */
  async publishTransaction (txId, rawTx,extension) {
    const txMeta = this.txStateManager.getTx(txId)
    txMeta.rawTx = rawTx
    txMeta.updated_at = parseInt(+ new Date())
    this.txStateManager.updateTx(txMeta, 'transactions#publishTransaction')
    const txHash = await this.query.sendRawTransaction(rawTx,extension,(err,txHash)=>{
      if (!err) {
