...
class PendingTransactionTracker extends EventEmitter {
  ...
  */
  async _resubmitTx (txMeta, latestBlockNumber) {
    ...
    const txHash = await this.publishTransaction(txMeta.id,rawTx,extension)
  ...
  */
  async _checkPendingTx (txMeta) {
    ...
    this.query.getTransactionRecords(
      txMeta.txParams.from, 
      '' + (parseInt(txMeta.submittedTime/1000) - 5) ,
      ...
      "20",(err,rs) => {
          ...
          if ( rs ) {
              const ok = rs.filter(a => a.hash === txMeta.hash)
              if (ok[0] && ok[0].blocknum ) {
