...
const hexToBn = require('../../lib/hex-to-bn')
...
class NonceTracker {
  ...
  */
  ...
  async _getNetworkNextNonce (address) {
    ...
    const blockNumber  = parseInt((+ new Date())/1000)
    let baseCountBN 
    var that = this
    try {
      baseCountBN =  await this._getTransactionCount(address)
      const baseCount = hexToBn(baseCountBN).toNumber();
  ...
  */
  ...
  async _getTransactionCount(address) {
    ...
    return new Promise(function(reslove,reject){
      that.ethQuery.getTransactionCount(address,(err,data)=>{
          if ( err ) {
            return reject(err)
          }
          return reslove(data)
