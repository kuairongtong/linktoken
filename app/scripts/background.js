...
// LINKTOKEN_TEST_CONFIG is used in e2e tests to set the default network to localhost
const firstTimeState = Object.assign({}, rawFirstTimeState, global.LINKTOKEN_TEST_CONFIG)

const STORAGE_KEY = 'krtnt-config'
...
global.LINKTOKEN_NOTIFIER = notificationManager
 ...
 * @property {MetaMaskState} data - The data emitted from Linker controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the Linker controller, and sets up all platform configuration.
...
async function initialize () {
  ...
  log.debug('Linker initialization complete.')
}
 ...
 * @returns {Promise<MetaMaskState>} Last data emitted from previous instance of Linker.
 */
async function loadStateFromPersistence () {
  ...
  if (versionedData && !versionedData.data) {
    ...
    if (diskStoreState && diskStoreState.data) {
      ...
      sentry.captureMessage('Linker - Empty vault found - recovered from diskStore', {
      ...
      sentry.captureMessage('Linker - Empty vault found - unable to recover')
    }
  }
  ...
  if (!versionedData) {
    throw new Error('Linker - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('Linker - Localstore not supported')
    })
  }
...
/**
 * Initializes the Linker Controller with any initial state and default language.
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('Linker - Persistence pipeline failed', error)
    }
  )
  ...
  async function persistData (state) {
    if (!state) {
      throw new Error('Linker - updated state is missing', state)
    }
    if (!state.data) {
      throw new Error('Linker - updated state does not have data', state)
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the Linker controller via a multiplexed duplex stream.
   * This method identifies trusted (Linker) interfaces, and connects them differently from untrusted (web pages).
  ...
  function connectRemote (remotePort) {
    ...
    if (isMetaMaskInternalProcess) {
      ...
      controller.setupTrustedCommunication(portStream, 'Linker')
...
// On first install, open a new tab with Linker
