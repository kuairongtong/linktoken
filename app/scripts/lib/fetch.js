...
function createFetchConfigFromReq({ req, rpcUrl, originHttpHeaderKey }) {
  ...
  parsedUrl.path = '/' + req.method.replace('eth_','')
  ...
  const serializedPayload = payload.method === 'eth_getTransactionRecords' ?  JSON.stringify(payload.params) : JSON.stringify(payload)
